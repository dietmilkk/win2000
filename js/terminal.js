(function() {
    ...
    var commands = {
        ...
        ipconfig: function() {
            ...
            var base = '\nConfiguracao IP do Windows 2000\n\n' +
                '        Nome do Host . . . . . . . : ' + host + '\n' +
                '        Sufixo DNS Primario . . . : \n' +
                '        Tipo de No . . . . . . . : Hibrido\n' +
                '        Roteamento IP Ativado. . . : Sim\n' +
                '        Proxy WINS Ativado. . . . : Nao\n\n' +
                'Adaptador Ethernet Conexao Local:\n\n' +
                '        Sufixo DNS especifico . . : \n' +
                '        Descricao . . . . . . . . : AMD PCNET Family PCI Ethernet Adapter\n' +
                '        Endereco Fisico . . . . . : 00-1A-2B-3C-4D-5E\n' +
                '        DHCP Ativado. . . . . . . : Sim\n' +
                '        Endereco IP . . . . . . . : 192.168.0.1\n' +
                '        Mascara de Sub-rede . . . : 255.255.255.0\n' +
                '        Gateway Padrao . . . . . . : 192.168.0.254\n' +
                '        Servidores DNS . . . . . . : 8.8.8.8\n' +
                '                                         8.8.4.4\n\n' +
                'Consultando IP externo...';
            printPre(base);
                ...
                if (xhr.status === 200 && xhr.responseText) {
                    try {
                        ...
                        if (d.status === 'success') {
                            ...
                            printPre('\n        IP Externo: indisponivel (' + (d.message || 'erro') + ')');
                        }
                    } catch(e) {
                        printPre('\n        IP Externo: erro ao processar resposta');
                    }
                } else {
                    printPre('\n        IP Externo: falha na consulta');
                }
            };
            xhr.onerror = function() {
                printPre('\n        IP Externo: servidor inacessivel');
            };
            xhr.ontimeout = function() {
                printPre('\n        IP Externo: tempo limite excedido');
            };
            xhr.send();
            return '';
        },
        ping: function(args) {
            ...
            for (var i = 0; i < 4; i++) {
                results.push('Resposta de ' + target + ': bytes=32 tempo=' + times[i] + 'ms TTL=128');
            }
            var min = Math.min.apply(null, times);
            var max = Math.max.apply(null, times);
            var avg = Math.floor(times.reduce(function(a, b) { return a + b; }, 0) / times.length);
            results.push('\nEstatisticas do ping para ' + target + ':');
            results.push('    Pacotes: Enviados = 4, Recebidos = 4, Perdidos = 0 (0% de perda),');
            results.push('Tempo aproximado em milissegundos:');
            results.push('    Minimo = ' + min + 'ms, Maximo = ' + max + 'ms, Medio = ' + avg + 'ms');
            return results.join('\n');
        },
        uptime: function() {
            return 'Tempo de atividade: ' + getUptime() + '\n' +
                'O sistema esta funcionando perfeitamente.';
        },
        data: function() {
            var now = new Date();
            ...
            var info = '=== Informacoes do Sistema ===\n\n' +
            ...
            printPre(info);
            printPre('  Buscando IP externo...');
            var xhr = new XMLHttpRequest();
            xhr.open('GET', 'https://api.ipify.org');
            xhr.timeout = 5000;
            xhr.onload = function() {
                if (xhr.status === 200 && xhr.responseText) {
                    printPre('  IP Externo: ' + xhr.responseText);
                }
            };
            xhr.send();
            return '';
    ...
    function processCommand(cmd) {
        ...
        if (handler) {
            ...
            printLine('"' + command + '" nao e reconhecido como um comando interno\nexterno, programa ou arquivo batch.');
        }
    }
    ...
    var termBehavior = new WindowBehavior(termWin, {
        ...
        minW: 500,
        minH: 300,
        taskbarIcon: '<img src="assets/icons/tango2kde/16x16/apps/terminal.png" alt="" width="14" height="14" style="flex-shrink:0;">',
